...
function HeroSection() {
    const [imageSize, setImageSize] = useState(1270); // Initial size of the hero image
    ...
    useEffect(() => {
        ...
        const initialSize = 1270;
        const sizeChangeFactor = 0.5; 
        const newSize = initialSize - scrollY * sizeChangeFactor;
        setImageSize(Math.max(newSize, 300));
    }, [scrollY]);

    return (
        <div className='h-[calc(80vh-64px)] md:h-[calc(100vh-72px)]  bg-primary relative'>
            ...
            <div className='absolute -bottom-36 md:-bottom-[700px] w-full'>
                <div className='flex w-full justify-center px-4'>
                    <Image src={'/HeroImage.png'} alt="logo" width={imageSize} height={370} />
                </div>
            </div>
        </div>
    )
